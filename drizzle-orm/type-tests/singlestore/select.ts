...
const leftJoinMixed = await db
	.select({
		id: users.id,
		text: users.text,
		textUpper: sql<string | undefined>`upper(${users.text})`,
...

{
	...
	const leftLateralRawRes = await db.select({
		table1,
		sqId: sql<number | undefined>`${sql.identifier('t2')}.${sql.identifier('id')}`.as('sqId'),
